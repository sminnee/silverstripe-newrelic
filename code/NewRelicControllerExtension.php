...
class NewRelicControllerExtension extends Extension {
	...
	function __destruct() {
		if($this->transactionName) {
			...
			if (extension_loaded('newrelic')) newrelic_name_transaction($this->transactionName);

			if($memberID = Session::get('loggedInAs')) {
				//Debug::message("newrelic_add_custom_parameter('memberID', $memberID)");
				if(extension_loaded('newrelic')) newrelic_add_custom_parameter('memberID', $memberID);
			}
		}
	}
}
